...
function av_get_verify_form() {
	...
	$submit_button_label = apply_filters( 'av_form_submit_label', __( 'Enter Site &raquo;', 'gn-age-verify' ) );
	...
	if ( $error ) :
		...
		// Catch-all error
		$error_string = apply_filters( 'av_error_text_general', __( 'Sorry, something must have gone wrong. Please try again', 'gn-age-verify' ) );
		
		// Visitor didn't check the box (only for the simple checkbox form)
		if ( $error == 2 )
			$error_string = apply_filters( 'av_error_text_not_checked', __( 'Check the box to confirm your age before continuing', 'gn-age-verify' ) );
		
		// Visitor isn't old enough
		if ( $error == 3 )
			$error_string = apply_filters( 'av_error_text_too_young', __( 'Sorry, it doesn\'t look like you\'re old enough', 'gn-age-verify' ) );
		
		// Visitor entered an invalid date
		if ( $error == 4 )
			$error_string = apply_filters( 'av_error_text_bad_date', __( 'Please enter a valid date', 'gn-age-verify' ) );
	...
	switch ( $input_type ) {
		...
		case 'checkbox' :
			...
			$form .= esc_html( sprintf( apply_filters( 'av_confirm_text', __( 'I am at least %s years old', 'gn-age-verify' ) ), av_get_minimum_age() ) ) . '</label></p>';
