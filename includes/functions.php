 ...
 * @since 0.2.6
 *
 * @package GN_Age_Verify\Functions
 */
 ...
 */
function gn_av_get_minimum_age() {
	 ...
	 */
	$minimum_age = apply_filters( 'gn_av_minimum_age', $minimum_age );
 ...
 */
function gn_av_get_visitor_age( $year, $month, $day ) {
 ...
 */
function gn_av_get_cookie_duration() {
	 ...
	 */
	$cookie_duration = (int) apply_filters( 'gn_av_cookie_duration', $cookie_duration );
 ...
 */
function gn_av_only_content_restricted() {
	 ...
	 */
	$only_content_restricted = apply_filters( 'gn_av_only_content_restricted', $only_content_restricted );
 ...
 */
function gn_av_content_is_restricted( $id = null ) {
	 ...
	 */
	$is_restricted = apply_filters( 'gn_av_is_restricted', $is_restricted, $id );
 ...
 * @return bool
 */
function gn_av_needs_verification() {
 ...
 * @return string|false
 */
function gn_av_get_the_desc() {

	$desc = apply_filters( 'gn_av_description', get_option( '_gn_av_description', __( 'Please verify your age', 'gn-age-verify' ) ) );
 ...
 * @return string
 */
function gn_av_get_input_type() {

	return apply_filters( 'gn_av_input_type', get_option( '_gn_av_input_type', 'dropdowns' ) );
}
 ...
 * @return string
 */
function gn_av_get_overlay_color() {
	...
	return apply_filters( 'gn_av_overlay_color', $color );
}
 ...
 * @return string
 */
function gn_av_get_background_color() {
	...
	return apply_filters( 'gn_av_background_color', $color );
}
...
function gn_av_get_verify_form() {
	...
	if ( $error ) :
		...
		// Catch-all error
		$error_string = apply_filters( 'gn_av_error_text_general', __( 'Sorry, something must have gone wrong. Please try again', 'gn-age-verify' ) );

		// Visitor didn't check the box (only for the simple checkbox form)
		if ( $error == 2 )
			$error_string = apply_filters( 'gn_av_error_text_not_checked', __( 'Check the box to confirm your age before continuing', 'gn-age-verify' ) );

		// Visitor isn't old enough
		if ( $error == 3 )
			$error_string = apply_filters( 'gn_av_error_text_too_young', __( 'Sorry, it doesn\'t look like you\'re old enough', 'gn-age-verify' ) );

		// Visitor entered an invalid date
		if ( $error == 4 )
			$error_string = apply_filters( 'gn_av_error_text_bad_date', __( 'Please enter a valid date', 'gn-age-verify' ) );
	...
	return apply_filters( 'gn_av_verify_form', $form );
}
 ...
 * @return bool
 */
function gn_av_confirmation_required() {
	...
	return (bool) apply_filters( 'gn_av_confirmation_required', $return );
}
 ...
 * @return bool
 */
function gn_av_register_check( $login, $email, $errors ) {

	if ( ! isset( $_POST['_gn_av_confirm_age'] ) )
		$errors->add( 'empty_age_confirm', '<strong>ERROR</strong>: ' . apply_filters( 'gn_av_registration_error', __( 'Please confirm your age', 'gn-age-verify' ) ) );
}
